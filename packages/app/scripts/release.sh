#!/usr/bin/env bash
# release.sh — TAG-DRIVEN release entrypoint for Clarus.
#
# Branching model "C":
#   1. Cut a release branch off <from> (default: dev).
#   2. Bump version via bump-version.mjs (writes tauri.conf.json / package.json /
#      root package.json / Cargo.toml / Cargo.lock).
#   3. Commit the bump.
...
#   - node available (used to run bump-version.mjs and read the current version).
...
APP_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"       # packages/app
...
CURRENT="$(node -e "console.log(JSON.parse(require('fs').readFileSync('$TAURI_CONF','utf8')).version)")"
...
# 4. Fetch origin and verify the base branch exists remotely.
step "Fetching origin (prune stale refs)"
git -C "$REPO_ROOT" fetch origin --prune

if ! git -C "$REPO_ROOT" ls-remote --exit-code --heads origin "$FROM_BRANCH" >/dev/null 2>&1; then
...
run "git -C '$REPO_ROOT' switch -c '$TEMP_BRANCH' 'origin/$FROM_BRANCH'"

step "Bumping version ($KIND)"
if [ "$DRY" = "1" ]; then
  # Compute what the bump would produce without writing any files, reusing the
  # exported nextVersion() so dry-run stays faithful to the real bump logic.
  NEXT="$(node -e "import('$SCRIPT_DIR/bump-version.mjs').then(m => process.stdout.write(m.nextVersion('$CURRENT', '$KIND')))")"
  c_dim "DRY: (cd '$APP_ROOT' && node scripts/bump-version.mjs '$KIND')  # would produce $NEXT"
else
...
run "git -C '$REPO_ROOT' branch -m '$TEMP_BRANCH' '$RELEASE_BRANCH'"
...
if git -C "$REPO_ROOT" rev-parse -q --verify "refs/tags/v$NEXT" >/dev/null 2>&1; then
  if [ "$DRY" = "0" ]; then
    git -C "$REPO_ROOT" switch --detach HEAD 2>/dev/null || true
...
if git -C "$REPO_ROOT" ls-remote --tags origin "refs/tags/v$NEXT" 2>/dev/null | grep -q "refs/tags/v$NEXT"; then
  if [ "$DRY" = "0" ]; then
    git -C "$REPO_ROOT" switch --detach HEAD 2>/dev/null || true
    git -C "$REPO_ROOT" branch -D "$RELEASE_BRANCH" 2>/dev/null || true
  fi
...
# -a stages all modified tracked files repo-wide (tauri.conf.json, both
# package.json files, Cargo.toml, Cargo.lock) regardless of cwd.
run "git -C '$REPO_ROOT' commit -am 'chore: release v$NEXT'"
...
MERGE_SHA=""
if [ "$DRY" = "0" ]; then
  MERGE_SHA="$(git -C "$REPO_ROOT" rev-parse origin/master)"
  c_dim "  Merge commit: $MERGE_SHA"
else
  c_dim "DRY: MERGE_SHA=\$(git -C '$REPO_ROOT' rev-parse origin/master)"
fi

# ---------- tag the merge commit and push (triggers CI) -----------------------

step "Tagging merge commit v$NEXT and pushing (this triggers CI)"
run "git -C '$REPO_ROOT' tag 'v$NEXT' '${MERGE_SHA:-<MERGE_SHA>}'"
run "git -C '$REPO_ROOT' push origin 'v$NEXT'"
...
if git -C "$REPO_ROOT" show-ref --verify --quiet "refs/heads/$DEV_BRANCH"; then
  run "git -C '$REPO_ROOT' switch '$DEV_BRANCH'"
  run "git -C '$REPO_ROOT' pull --ff-only origin '$DEV_BRANCH'"
else
  run "git -C '$REPO_ROOT' switch -c '$DEV_BRANCH' 'origin/$DEV_BRANCH'"
fi

BACK_MERGE_OK=1
if [ "$DRY" = "1" ]; then
  run "git -C '$REPO_ROOT' merge --no-ff origin/master -m 'chore: back-merge v$NEXT into dev'"
  run "git -C '$REPO_ROOT' push origin '$DEV_BRANCH'"
else
  if git -C "$REPO_ROOT" merge --no-ff origin/master -m "chore: back-merge v$NEXT into dev"; then
    if ! git -C "$REPO_ROOT" push origin "$DEV_BRANCH"; then
      BACK_MERGE_OK=0
    fi
  else
    BACK_MERGE_OK=0
