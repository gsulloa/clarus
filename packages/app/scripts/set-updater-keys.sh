...
#   - Private key → local file at $KEY_PATH (default ~/.tauri/clarus-updater.key)
...
#
# Env overrides:
#   UPDATER_REPO         — GitHub repo slug (default: gsulloa/clarus)
#   UPDATER_KEY_PATH     — local key file path (default: ~/.tauri/clarus-updater.key)
...
REPO="${UPDATER_REPO:-gsulloa/clarus}"
KEY_PATH="${UPDATER_KEY_PATH:-$HOME/.tauri/clarus-updater.key}"
...
if $rotate || [ ! -f "$KEY_PATH" ]; then
  ...
  pnpm --filter clarus exec tauri signer generate \
    -w "$KEY_PATH" \
    -p "$PASSWORD" \
    --ci \
    --force
